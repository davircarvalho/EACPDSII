...
if size(input,2)>1 && size(input,1)~=1
   disp('Make sure the input data is organized in columns...');
elseif size(input,1)==1   
   disp('The input data must be organized in columns... I am transposing your input.') 
   input = input.';
end

L = size(input,1); CI.dataLength = L;
a = 1 - alpha/100;
